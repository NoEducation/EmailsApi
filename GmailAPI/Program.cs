...
namespace GmailAPI
{
    public class Program
    {
        ...
        public static List<GmailMail> GetAllEmails(string HostEmailAddress)
        {
            try
            {
                GmailService GmailService = GmailAPIHelper.GetService();
                List<GmailMail> EmailList = new List<GmailMail>();
                ...
                if (ListResponse != null && ListResponse.Messages != null)
                {
                    ...
                    foreach (Message Msg in ListResponse.Messages)
                    {
                        ...
                        if (MsgContent != null)
                        {
                            ...
                            {
                                GmailMail GMail = new GmailMail();
                                GMail.From = FromAddress;
                                GMail.Body = ReadableText;
                                GMail.MailDateTime = DateTime.SpecifyKind(DateTime.Parse(Date), DateTimeKind.Utc);
                                EmailList.Add(GMail);
                            }
                        }
                    }
